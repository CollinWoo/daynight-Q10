...
```{r Q10 Function}
# p-value function from https://gettinggeneticsdone.blogspot.com/2011/01/rstats-function-for-extracting-f-test-p.html
...
q10 <- 2
x <- 1:20
y <- q10 ^ (x/10) + rnorm(length(x), sd = .3)
plot(x,y)
cat("The Q10 Value should be", q10, "and is", calc_q10(x,y)[,1])

df <- data.frame()
for(q10 in c(1.5, 2, 3)) {
  df <- bind_rows(df, tibble(Q10 = q10,
                             Temperature = x,
                             Respiration = q10 ^ (x / 10) + rnorm(length(x), sd = 0.3),
                             Prediction = exp(predict(lm(log(Respiration) ~ Temperature)))))
}
df %>% 
  mutate(Q10 = as.factor(Q10)) %>% 
  ggplot(aes(Temperature, Respiration, color = Q10)) +
  geom_point(size = 2) +
  geom_line(aes(y = Prediction), linetype = 2)
```
...
a <- ggplot(week_Q10, aes(x = R2)) + geom_histogram(color= "black", fill = "white") + labs(title = "R2 Distribution")
ggsave("R2 Distribution.png", a)
a2 <- ggplot(week_Q10, aes(x = Q10)) + geom_histogram(color= "black", fill = "white") + xlim(0, 6) + labs(title = "Q10 Distribution")
ggsave("Q10 Distribution.png", a2)
a3 <- ggplot(week_Q10, aes(x = Q10)) + geom_histogram(color= "black", fill = "white") + labs(title = "Q10 Distribution by Rs/Rh") + xlim(0, 6) + facet_wrap(~Rs_Rh)
...
c <- ggplot(week_Q10, aes(x = Q10, fill=DAY_NIGHT)) + geom_histogram(position = "dodge2") + xlim(0, 6) + facet_wrap(~DATASET) + labs(title = "Q10 By Dataset")
...
d <- ggplot(month_Q10, aes(x = MONTH, y = DATASET, fill = Q10)) + geom_tile() + lims(fill = c(0, 6))
ggsave("tile1.png", d)
d2 <- ggplot(monthyear_Q10, aes(x = MONTH, y = YEAR, fill = Q10)) + geom_tile() + lims(fill = c(0, 6))
ggsave("tile2.png", d2)
print(d)
print(d2)
```
...
for(depth in Shared_depths_list){
  Rs_table <- week_Q10 %>% filter(Rs_Rh == "Rs", DEPTH == as.numeric(depth))
  print(ggplot(Rs_table, aes(MON_YEAR, Q10, color = DAY_NIGHT, group = paste(YEAR, DAY_NIGHT))) + geom_point() + ylim(0,6) + facet_wrap(~DATASET, scales = "free_x") + labs(title = paste("Rs Variation by Day/Night at depth", depth)))
  print(ggplot(week_Q10 %>% filter(Rs_Rh == "Rh", DEPTH == as.numeric(depth), DATASET %in% Rs_table$DATASET), aes(MON_YEAR, Q10, color = DAY_NIGHT, group = paste(YEAR, DAY_NIGHT))) + geom_point() + ylim(0,6) + facet_wrap(~DATASET, scales = "free_x") + labs(title = paste("Rh Variation by Day/Night at depth", depth)) + theme(
    strip.background = element_blank(),
    strip.text.x = element_text(size = 10)
  ))
  print(ggplot(week_Q10 %>% filter(DEPTH == as.numeric(depth), DATASET %in% Rs_table$DATASET), aes(MON_YEAR, Q10, color = DAY_NIGHT, group = paste(YEAR, DAY_NIGHT))) + geom_point() + ylim(0,6) + facet_grid(Rs_Rh~DATASET, scales = "free_x") + labs(title = paste("Rh and Rs Variation by Day/Night at depth", depth)) + 
          theme(
            strip.background = element_blank(),
            strip.text.x = element_text(size = 10)
          ))
...
for(depth in Shared_depths_list){
  ...
  print(ggplot(Rs_table, aes(MON_YEAR, Q10, color = Rs_Rh, group = paste(YEAR, DAY_NIGHT))) + geom_point() + ylim(0,6) + facet_wrap(~DATASET, scales = "free_x") + labs(title = paste("Day Variation by Rs/Rh at depth", depth)))
  print(ggplot(week_Q10 %>% filter(DAY_NIGHT == "night", DEPTH == as.numeric(depth), DATASET %in% Rs_table$DATASET), aes(MON_YEAR, Q10, color = Rs_Rh, group = paste(YEAR, DAY_NIGHT))) + geom_point() + ylim(0,6) + facet_wrap(~DATASET, scales = "free_x") + labs(title = paste("Night Variation by Rs/Rh at depth", depth)) + 
          theme(
            strip.background = element_blank(),
            strip.text.x = element_text(size = 10)
          ))
  print(ggplot(week_Q10 %>% filter(DEPTH == as.numeric(depth), DATASET %in% Rs_table$DATASET), aes(MON_YEAR, Q10, color = Rs_Rh, group = paste(YEAR, DAY_NIGHT))) + geom_point() + ylim(0,6) + facet_grid(DAY_NIGHT~DATASET, scales = "free_x") + labs(title = paste("Day and Night Variation by Rs/Rh at depth", depth)) + 
          theme(
            strip.background = element_blank(),
            strip.text.x = element_text(size = 10)
          ))
...
x2 <- ggplot(Rs_table, aes(MON_YEAR, Q10, color = DAY_NIGHT, group = paste(YEAR, DAY_NIGHT))) + geom_line() + ylim(0,6) + facet_wrap(~DATASET, scales = "free_x") + labs(title = paste("Rs Variation by Day/Night at depth", 5))
ggsave("Rs5.png", x2)
Rh_table <- week_Q10 %>% filter(Rs_Rh == "Rh", DEPTH == 5)
x3 <- ggplot(Rh_table, aes(MON_YEAR, Q10, color = DAY_NIGHT, group = paste(YEAR, DAY_NIGHT))) + geom_point() + ylim(0,6) + facet_wrap(~DATASET, scales = "free_x") + labs(title = paste("Rh Variation by Day/Night at depth", 5))
ggsave("Rh5.png", x3)
ggsave("Rs5.png", x2)

print(ggplot(week_Q10 %>% filter(DAY_NIGHT == "day"), aes(MON_YEAR, Q10, color = Rs_Rh, group = paste(YEAR, DAY_NIGHT))) + geom_point() + ylim(0,6) + facet_wrap(~DATASET+DEPTH, scales = "free_x") + labs(title = "Day Variation by Rs/Rh"))

print(ggplot(month_Q10, aes(MAT, MAP, color = Q10)) + geom_jitter() + lims(color = c(0, 6)))
```
```{r Site Map}
