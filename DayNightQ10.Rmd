...
SMOStifnames <- function(datevec){
  ...
  tifiles <- list.files("./L03/", full.names = TRUE, pattern = "as1.tif")
  
  # datevec may be long, but there are only so many *unique* dates
  unique_datevec <- unique(datevec)
  # set up result vector
  dateFiles <-c()
  
  for(i in seq_along(unique_datevec)){
    date <- unique_datevec[i]
    ...
    names(matchfiles) <- rep(as.character(date), length(matchfiles))
    
    dateFiles <- append(dateFiles, matchfiles)  # this is slow
  }
  
  if(length(dateFiles)) {
    results <- tibble(date = names(dateFiles), files = dateFiles)
    data.frame(date = as.character(datevec)) %>% 
      left_join(results, by = "date") %>% 
      pull(files)
  } else {  # if no matches at all, just return NA
    rep(NA_character_, length(datevec))
  }
}
...
for(rhds in rh_dataset_names[-9]){
  ...
  dsd %>% 
    ...
    mutate(WOY = week(CSR_TIMESTAMP_END),
           ...
           Rs_Rh = factor(Rs_Rh),
           Surface.File = SMOStifnames(CSR_TIMESTAMP_BEGIN)
    ) -> dsd
