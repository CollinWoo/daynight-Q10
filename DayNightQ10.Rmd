...
plot(moisture)
maps::map(add = T)
colnames(MA_row)[1:2] <- c("x","y")
print(MA_row)
testlocal <- raster::extract(moisture, MA_row[1:2])
print(testlocal)
...
SMOSfiles <- function(date){
  ...
  SMOShtml <- read_html("https://gimms.gsfc.nasa.gov/SMOS/jbolten/FAS/L03/")
  SMOShtml %>% html_nodes(".indexcolname") %>% html_text() -> filenames
  
  #convert to date
  fileDate <- as.Date(date)
  probDate <- c(fileDate-1, fileDate, fileDate+1)
  probDate <- lapply(probDate, as.Date)
  probDate <- lapply(probDate, format, "%Y%m%d")
...
SMOSfiles("2012/12/15")
SMOSfiles("2012/12/14")
```
