...
print(bio)
print(precip)
rh_dataset_names <- db_info[grep("Rh", db_info$CSR_MSMT_VAR), ][[1]]
...
calc_q10 <- function(temp, resp, out.rm = TRUE){
  ...
  if(length(resp) < 4){
    return(data.frame(Q10 = NA, R2 = NA, P_VALUE = NA))
  }
...
SMOStifnames <- function(datevec){
  ...
  for(date in fileDates){
    ...
    if(length(matchfiles) > 0){
      matchfiles <- paste0("./LO3_tif/", matchfiles)[[1]]
    }else{
        matchfiles <- NA
    }
    
    dateFiles <- append(dateFiles, matchfiles)
  }
...
hirano <- csr_dataset("d20200109_HIRANO_PDB")
hirano_data <- hirano$data
hirano_dates <- hirano_data$CSR_TIMESTAMP_BEGIN
SMOStifnames(c("2013/5/18", "2013/6/18", "2021/12/12", "2013/6/20", "2013/6/21"))
testFrame = data.frame(dates = c("2013/5/18", "2013/6/18", "2021/12/12", "2013/6/20", "2013/6/21"))
...
#List of datasets that measure Rs and have a $data data frame
db_info %>% 
  filter(!is.na(CSR_RECORDS) & grepl("Rs", CSR_MSMT_VAR)) %>%
  pull(CSR_DATASET) -> Rs_Datasets
print(Rs_Datasets)

for(rhds in Rs_Datasets){
  #extract necessary data frames from the csr_dataset
  ds_data <- csr_dataset(rhds)
  dsd <- ds_data$data
  ds_desc <- ds_data$description
  ds_ports <- ds_data$ports
  
  #get the column names of all temperature columns in the csr_dataset
  column_names <- names(dsd)
  temperature_columns <- column_names[grep("^CSR_T(AIR|[0-9]+)", column_names)]
  
  #Get port numbers that measure Rs and port numbers that measure Rh
  rh_ports <- final_results[final_results$Dataset == rhds, ][[2]]
  rs_ports <- ds_ports[ds_ports$CSR_MSMT_VAR == "Rs",][[1]]
  
  #Get the latitude, longitude, timezone, and igbp for this site
  ...
  #Calculate Sunset and sunrise times for this site
...
bind_rows(yearonly_Q10, .id = "ID") %>% 
  unpack(cols = Q10) %>%
  filter(!is.na(Q10), is.finite(Q10)) %>% 
  dplyr::select(DATASET, YEAR, DEPTH:RESP, IGBP, N) %>% 
  arrange(DATASET, DEPTH, YEAR) %>%
  left_join(MA_set, by = "DATASET") -> 
  yearonly_Q10
...
for(depth in Shared_depths_list){
  ...
  Rs_table %>% filter(!grepl("_MINIAT", DATASET)) %>%
  ...
  p <- ggplot(Rs_table, aes(MONTH, Q10, color = DAY_NIGHT, group = paste(YEAR, DAY_NIGHT))) +
  ...
  p <- ggplot(Rs_table, aes(MONTH, Q10, color = DAY_NIGHT, group = paste(YEAR, DAY_NIGHT))) + geom_point() + ylim(0,12.5) + 
    facet_wrap(~IGBP, scales = "free_x") +  labs(title = paste("Rs Variation by Day/Night at depth", depth))
  print(p)
  
  Rh_table <- week_Q10 %>% filter(DEPTH == as.numeric(depth), DATASET %in% Rs_table$DATASET)
  
  if("Rh" %in% Rh_table$Rs_Rh){
    p <- Rh_table %>% 
      filter(Rs_Rh == "Rh") %>% 
      ggplot(aes(MONTH, Q10, color = DAY_NIGHT, group = paste(YEAR, DAY_NIGHT))) +
      geom_point() + ylim(0,12.5) + 
      facet_wrap(~DATASET, scales = "free_x") + 
...
```{r, warning=FALSE, fig.width=10, eval=TRUE}
for(depth in Shared_depths_list){
  Rs_table <- week_Q10 %>% filter(DAY_NIGHT == "day", DEPTH == as.numeric(depth))
  
  print(ggplot(Rs_table, aes(MONTH, Q10, color = Rs_Rh, group = paste(YEAR, DAY_NIGHT))) + geom_point() + ylim(0,12.5) + facet_wrap(~DATASET, scales = "free_x") + labs(title = paste("Day Variation by Rs/Rh at depth", depth)))
  
  print(ggplot(week_Q10 %>% filter(DAY_NIGHT == "night", DEPTH == as.numeric(depth), DATASET %in% Rs_table$DATASET), aes(MONTH, Q10, color = Rs_Rh, group = paste(YEAR, DAY_NIGHT))) + geom_point() + ylim(0,12.5) + facet_wrap(~DATASET, scales = "free_x") + labs(title = paste("Night Variation by Rs/Rh at depth", depth)) + 
          theme(
            strip.background = element_blank(),
            strip.text.x = element_text(size = 10)
          ))
  
  print(ggplot(week_Q10 %>% filter(DEPTH == as.numeric(depth), DATASET %in% Rs_table$DATASET), aes(MONTH, Q10, color = Rs_Rh, group = paste(YEAR, DAY_NIGHT))) + geom_point() + ylim(0,12.5) + facet_grid(DAY_NIGHT~DATASET, scales = "free_x") + labs(title = paste("Day and Night Variation by Rs/Rh at depth", depth)) + 
          theme(
            strip.background = element_blank(),
            strip.text.x = element_text(size = 10)
          ))
