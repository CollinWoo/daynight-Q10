...
for(rhds in rh_dataset_names){
  ...
  MA_row <- data.frame(Latitude = lat, Longitude = lon)
  ...
  apply(tmean_vals, 1, mean) -> MA_row$MAT
...
bind_rows(week_Q10, .id = "ID") %>% 
  ...
  select(DATASET, YEAR:MONTH, DAY_NIGHT, Rs_Rh, DEPTH:RESP, N) %>% 
...
b <- ggplot(week_Q10, aes(x = R2, fill=DAY_NIGHT)) + geom_histogram() + facet_wrap(~DATASET+DAY_NIGHT)
c <- ggplot(week_Q10, aes(x = Q10, fill=DAY_NIGHT)) + geom_histogram() + xlim(0, 5) + facet_wrap(~DATASET+DAY_NIGHT)
