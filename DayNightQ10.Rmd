...
SMOStif <- function(datevec, lon, lat){
  ...
  dateFiles %>% rename(Surface = V4, Subsurface = V5) -> dateFiles
  print(dateFiles)
  return(dateFiles)
}
...
get_sm_data <- function(lon, lat, timestamps) {
  ...
  moisture_frame %>% mutate(Files = SMOSfiles(Times)) %>% unpack(cols=Files) %>% distinct(Soil.Surface, .keep_all = TRUE) %>% mutate(Soil.Surface = load_from_grib_file(Soil.Surface, lon, lat), Subsurface = load_from_grib_file(Subsurface, lon, lat)) -> moisture_frame
  print(moisture_frame)
