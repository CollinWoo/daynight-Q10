...
a3 <- week_Q10 %>% group_by(Rs_Rh) %>% summarise(medianval = median(Q10)) %>% right_join(week_Q10, by = "Rs_Rh") %>% ggplot(aes(x = Q10)) + geom_histogram(color= "black", fill = "white") + labs(title = "Q10 Distribution by Rs/Rh") + xlim(0, 6) + facet_wrap(~Rs_Rh) + stat_bin(geom="text", aes(label=..count..), size = 2, vjust = -1) + geom_vline(aes(xintercept = medianval), color = "red", linetype="dashed", size = 1) + geom_text(aes(label = paste("Median =", round(medianval, digits=3))), x = 5, y = 150, size = 3)
