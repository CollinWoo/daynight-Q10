...
for(rhds in rh_dataset_names){
  ...
  dsd %>% 
    mutate(DAY_NIGHT = case_when(
      ...
      TRUE ~ "night"), Rs_Rh = case_when(
        CSR_PORT %in% rh_ports ~ "Rh", 
        TRUE ~ "Rs")) %>%
    filter(!is.na(CSR_FLUX_CO2) & CSR_FLUX_CO2 > 0) %>%
    mutate(WOY = week(CSR_TIMESTAMP_END), YEAR = year(CSR_TIMESTAMP_END), DAY_NIGHT = as.factor(DAY_NIGHT), Rs_Rh = factor(Rs_Rh)) -> dsd
  ...
  for(tcol in temperature_columns){
    ...
    dsd %>% 
      ...
      summarize(DATASET = rhds, DEPTH = depth, Q10 = calc_q10(temp, CSR_FLUX_CO2), TEMP = mean(temp, na.rm=TRUE), RESP = mean(CSR_FLUX_CO2)) %>% 
      filter(!is.na(Q10)) %>% 
      mutate(DEPTH = factor(DEPTH))-> week_Q10[[paste(rhds, depth, sep="_")]]
  }
}
week_Q10 <- bind_rows(week_Q10, .id = "ID")
week_Q10 %>% 
  select(DATASET, YEAR:WOY, DAY_NIGHT, Rs_Rh, DEPTH:RESP) %>% 
  arrange(DATASET, DEPTH, YEAR, WOY) -> week_Q10
