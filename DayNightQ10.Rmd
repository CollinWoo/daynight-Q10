...
SMOSfiles <- function(date){
  #lastChecked = as.Date(readLines("lastChecked.txt"))
  ...
  if(length(matchfiles)==0){
    return(data.frame(Soil.Surface = "No Files", Subsurface = "No Files"))
  }
  fileurls <- paste0("https://gimms.gsfc.nasa.gov/SMOS/jbolten/FAS/L03/", matchfiles)
  matchfiles <- paste0("./L03/", matchfiles)
  
  if(!file.exists(matchfiles[1])){
    download.file(fileurls[1], destfile= paste0(matchfiles[1]), method = "curl")
    download.file(fileurls[2], destfile= paste0(matchfiles[2]), method = "curl")
  }
  names(matchfiles) = c("Soil Surface", "Subsurface")
  return(data.frame(as.list(matchfiles)))
}

# Extract soil moisture data for a given {lon,lat} from `filename`
extractSMOS <- function(filename, lon, lat){
  sp <- SpatialPoints(cbind(lon, lat), proj4string = CRS("+proj=longlat +datum=WGS84 +no_defs"))
  raster::extract(raster(filename), sp)
}


# Build NASA-USDA Soil Moisture filenames from a vector of dates
SMOStifnames <- function(datevec){
  stopifnot(all(is.Date(datevec)))
  tifiles <- list.files("./LO3_tif/", full.names = TRUE, pattern = "as1.tif")
  
  # datevec may be long, but there are only so many *unique* dates
  # set up the results data frame
  results <- tibble(date = unique(datevec), filename = NA_character_)
  
  for(i in seq_len(nrow(results))){
    date <- results$date[i]
    probDate <- format(c(date-1, date, date+1), "%Y%m%d")
    probDate[seq(1, length(probDate), 3)] <- paste0(probDate[seq(1, length(probDate), 3)], "_")
    probDate[seq(3, length(probDate), 3)] <- paste0("_", probDate[seq(3, length(probDate), 3)])
    patterns <- paste(probDate, collapse = "|")
    matchfile <- tifiles[grep(patterns, tifiles)]
    
    if(length(matchfile)){
      stopifnot(length(matchfile) < 2)  # there should be 0 or 1 matches only
      results$filename[i] <- matchfile
    }
  }
  
  # now that we have the filenames for our unique dates, merge with the full
  # `datevec` and return to caller
  tibble(date = datevec) %>% 
...
SMOStif <- function(datevec, lon, lat){
  ...
  bind_rows(dateFiles, .id = "Date") %>% 
    mutate(Surface = NA, Subsurface = NA) %>%
  ...
  if(nrow(dateFiles) > 0){
    for(value in 1:nrow(dateFiles)){
      dateFiles$Surface[value] = extractSMOS(dateFiles[[value, 2]], lon, lat)
      dateFiles$Subsurface[value] = extractSMOS(dateFiles[[value, 3]], lon, lat)
    }
  }
...
```{r Moisture Tests, eval = FALSE}
#SMOStif(c("2013/5/18", "2013/6/18", "2021/12/12", "2013/6/20", "2013/6/21"), 30, 60)
hirano <- csr_dataset("d20190424_ZHANG_maple")
hirano_data <- hirano$data
hirano_dates <- hirano_data$CSR_TIMESTAMP_BEGIN
print(as.Date(c("2013/5/18", "2013/6/18", "2021/12/12", "2013/6/20", "2013/6/21")))
SMOStifnames(as.Date(c("2013/5/18", "2013/6/18", "2021/12/12", "2013/6/20", "2013/6/21")))
testFrame = data.frame(dates = as.Date(hirano_dates))
testFrame %>% mutate(Surface.File = SMOStifnames(dates)) -> testFrame
print(testFrame)
testMoisture = SMOStif(testFrame$dates, 1, 1)
print(testMoisture)
testFrame %>% left_join(testMoisture, by="Surface.File") -> testFrame
print(testFrame)
```

## Ts Percentage
```{r Ts Percentage, eval=FALSE}
main_results <- list()
for(rhds in rh_dataset_names){
  ds_data <- csr_dataset(rhds)
  dsd <- ds_data$data
...
for(rhds in rh_dataset_names[-9]){
  ds_data <- csr_dataset(rhds)
  dsd <- ds_data$data
  ...
  igbp <- db_info$CSR_IGBP[db_info$CSR_DATASET == rhds]
  ...
  apply(precip_coords, 1, sd) -> MA_row$PSD
  MA_row$PSD/MA_row$MAP -> MA_row$PCV
  
  # The same for MAT
  raster::extract(tmean, MA_row[1:2]) -> tmean_vals
  print(tmean_vals)
  apply(tmean_vals, 1, mean)/10 -> MA_row$MAT
  apply(tmean_vals/10, 1, sd) -> MA_row$TSD
   MA_row$TSD/MA_row$MAT -> MA_row$TCV
  ...
  dsd %>% 
    ...
    mutate(WOY = week(CSR_TIMESTAMP_END),
           ...
           Rs_Rh = factor(Rs_Rh),
           Surface.File = SMOStifnames(as.Date(CSR_TIMESTAMP_BEGIN))
    ) -> dsd
  
  #Table of moisture values based on file name
  moisture_table <- SMOStif(dsd$CSR_TIMESTAMP_BEGIN, lon, lat)
  dsd %>% left_join(moisture_table, by = "Surface.File") -> dsd
  ...
  for(tcol in temperature_columns){
    ...
    dsd %>% 
      ...
      summarize(DATASET = rhds, 
                ...
                N = n()) %>% 
      unpack(cols = Q10) %>%
      mutate(MON_YEAR = paste(YEAR, MONTH, 15, sep = "/"), 
             MON_YEAR = as.Date(MON_YEAR)
      ) -> 
      week_Q10[[paste(rhds, depth, sep="_")]]
    
    dsd %>%
      group_by(YEAR, DAY_NIGHT, Rs_Rh) %>%
      summarize(DATASET = rhds,
                DEPTH = depth,
                Q10 = calc_q10(temp, CSR_FLUX_CO2, out.rm=TRUE),
                TEMP = mean(temp, na.rm=TRUE),
                ...
                N = n()) %>%
      unpack(cols = Q10) ->
      year_Q10[[paste(rhds, depth, sep="_")]]
    ...
    dsd %>%
      ...
      summarize(DATASET = rhds,
                ...
                N = n()) %>%
      unpack(cols = Q10) ->
      yearonly_Q10[[paste(rhds, depth, sep="_")]]
  }
}
...
bind_rows(week_Q10, .id = "ID") %>% 
  ...
  dplyr::select(DATASET, YEAR:MONTH, MON_YEAR, DAY_NIGHT, Rs_Rh, DEPTH:RESP, IGBP, N) %>% 
  arrange(DATASET, DEPTH, YEAR, MONTH) %>%
  left_join(MA_set, by = "DATASET") -> 
  week_Q10

bind_rows(year_Q10, .id = "ID") %>% 
  filter(!is.na(Q10), is.finite(Q10)) %>% 
  dplyr::select(DATASET, YEAR, DAY_NIGHT, Rs_Rh, DEPTH:RESP, IGBP, N) %>% 
  arrange(DATASET, DEPTH, YEAR) %>%
  left_join(MA_set, by = "DATASET") -> 
  year_Q10

bind_rows(yearonly_Q10, .id = "ID") %>% 
  filter(!is.na(Q10), is.finite(Q10)) %>% 
  dplyr::select(DATASET, YEAR, DEPTH:RESP, SURFACE, SUBSURFACE, IGBP, N) %>% 
  arrange(DATASET, DEPTH, YEAR) %>%
  left_join(MA_set, by = "DATASET") -> 
  yearonly_Q10
...
#rdistr = geom_histogram(color= "black", fill = "white") + geom_vline(aes(xintercept = median(R2)), linetype="dashed", color="red") 
monthR2 <- ggplot(week_Q10, aes(x = R2)) + geom_histogram(color= "black", fill = "white") + geom_vline(aes(xintercept = median(R2)), linetype="dashed", color="red") + labs(title = "R2 Distribution") + geom_text(aes(label = paste("Median =", round(median(R2), digits=3))), x = 0.15, y = 400, size = 3)
ggsave("R2 Distribution.png", monthR2)

yearR2 <- ggplot(year_Q10, aes(x = R2)) + geom_histogram(color= "black", fill = "white") + geom_vline(aes(xintercept = median(R2)), linetype="dashed", color="red") + labs(title = "Year R2 Distribution") + geom_text(aes(label = paste("Median =", round(median(R2), digits=3))), x = 0.3, y = 20, size = 3)
ggsave("Year R2 Distribution.png", yearR2)

IGBPR2month <- week_Q10 %>% filter(Rs_Rh=="Rs") %>% group_by(IGBP) %>% summarise(medianval = median(R2)) %>% right_join(week_Q10 %>% filter(Rs_Rh=="Rs"), by = "IGBP") %>% ggplot(aes(x = R2)) + geom_histogram(color= "black", fill = "white") + labs(title = "Month Rs R2 Distribution by IGBP") + facet_wrap(~IGBP, scale="free") + stat_bin(geom="text", aes(label=..count..), size = 2, vjust = -1) + geom_vline(aes(xintercept = medianval), color = "red", linetype="dashed", size = 1) + geom_text(aes(label = paste("Median =", round(medianval, digits=3))), x = 0.3, y = 4, size = 3)
ggsave("IGBP R2 Distribution.png", IGBPR2month)

IGBPR2year <- year_Q10 %>% filter(Rs_Rh=="Rs") %>% group_by(IGBP) %>% summarise(medianval = median(R2)) %>% right_join(year_Q10 %>% filter(Rs_Rh=="Rs"), by = "IGBP") %>% ggplot(aes(x = R2)) + geom_histogram(color= "black", fill = "white") + labs(title = "Year Rs R2 Distribution by IGBP") + facet_wrap(~IGBP, scale="free") + stat_bin(geom="text", aes(label=..count..), size = 2, vjust = -1) + geom_vline(aes(xintercept = medianval), color = "red", linetype="dashed", size = 1) + geom_text(aes(label = paste("Median =", round(medianval, digits=3))), x = 0.3, y = 0.75, size = 3)
ggsave("IGBP R2 Year Distribution.png", IGBPR2year)
...
yearQ10 <- ggplot(year_Q10, aes(x = Q10)) + geom_histogram(color= "black", fill = "white") + xlim(0, 12.5) + labs(title = "Year Q10 Distribution") + stat_bin(geom="text", aes(label=..count..), size = 2, vjust = -1) + geom_vline(aes(xintercept=median(Q10)), linetype="dashed") + geom_text(aes(label = paste("Median =", round(median(Q10), digits=3))), x = 5, y = 40, size = 3) + geom_text(aes(label = paste("Mean =", round(mean(Q10), digits=3))), x = 5, y = 50, size = 3)
ggsave("Year Q10 Distribution.png", yearQ10)
...
RsRhYear <- year_Q10 %>% group_by(Rs_Rh) %>% summarise(medianval = median(Q10)) %>% right_join(week_Q10, by = "Rs_Rh") %>% ggplot(aes(x = Q10)) + geom_histogram(color= "black", fill = "white") + labs(title = "Year Q10 Distribution by Rs/Rh") + xlim(0, 12.5) + facet_wrap(~Rs_Rh) + stat_bin(geom="text", aes(label=..count..), size = 2, vjust = -1) + geom_vline(aes(xintercept = medianval), color = "red", linetype="dashed", size = 1) + geom_text(aes(label = paste("Median =", round(medianval, digits=3))), x = 5, y = 150, size = 3)
ggsave("Rh-Rs Year Q10 Distribution.png", RsRhYear)

IGBPmonth <- week_Q10 %>% filter(Rs_Rh=="Rs") %>% group_by(IGBP) %>% summarise(medianval = median(Q10)) %>% right_join(week_Q10 %>% filter(Rs_Rh=="Rs"), by = "IGBP") %>% ggplot(aes(x = Q10)) + geom_histogram(color= "black", fill = "white") + labs(title = "Month Rs Q10 Distribution by IGBP") + xlim(0, 12.5) + facet_wrap(~IGBP, scale="free_y") + stat_bin(geom="text", aes(label=..count..), size = 2, vjust = -1) + geom_vline(aes(xintercept = medianval), color = "red", linetype="dashed", size = 1) + geom_text(aes(label = paste("Median =", round(medianval, digits=3))), x = 8, y = 10, size = 3)
ggsave("IGBP Q10 Distribution.png", IGBPmonth)

IGBPyear <- year_Q10 %>% filter(Rs_Rh=="Rs") %>%  group_by(IGBP) %>% summarise(medianval = median(Q10)) %>% right_join(week_Q10 %>% filter(Rs_Rh=="Rs"), by = "IGBP") %>% ggplot(aes(x = Q10)) + geom_histogram(color= "black", fill = "white") + labs(title = "Year Rs Q10 Distribution by IGBP") + xlim(0, 12.5) + facet_wrap(~IGBP, scale="free_y") + stat_bin(geom="text", aes(label=..count..), size = 2, vjust = -1) + geom_vline(aes(xintercept = medianval), color = "red", linetype="dashed", size = 1) + geom_text(aes(label = paste("Median =", round(medianval, digits=3))), x = 8, y = 10, size = 3)
ggsave("Year IGBP Q10 Distribution.png", IGBPyear)
...
ggplot(yearonly_Q10, aes(TEMP, SURFACE)) + geom_point(aes(size=Q10)) + ylim(0, 25) + xlim(0, 25)
ggsave("Moistureplot1.png")

ggplot(yearonly_Q10, aes(TEMP, SURFACE)) + geom_point(aes(size=Q10)) + ylim(9990, 10010)
ggplot(yearonly_Q10, aes(TEMP, SURFACE)) + geom_point(aes(size=Q10))

ggplot(year_Q10 %>% filter(DEPTH==-1, Rs_Rh == "Rh"), aes(PCV, Q10)) + geom_point(aes(color=DATASET))
ggplot(year_Q10 %>% filter(DEPTH==-1, Rs_Rh == "Rh"), aes(TCV, Q10)) + geom_point(aes(color=DATASET))
```
...
get_sm_data <- function(lon, lat, timestamps) {
  moisture_frame = data.frame(Times = timestamps)
  print(moisture_frame)
  moisture_frame %>% mutate(Files = SMOSfiles(Times)) %>% 
    unpack(cols=Files) %>% 
    distinct(Soil.Surface, .keep_all = TRUE) %>% 
    mutate(Soil.Surface = load_from_grib_file(Soil.Surface, lon, lat), Subsurface = load_from_grib_file(Subsurface, lon, lat), Lon = lon, Lat = lat)  -> moisture_frame
  print(moisture_frame)
  #fill method may be useful
}

library(lubridate)
timestamps <- seq(ymd_hms("2020-10-22 16:42:00"), by = 1000, length.out = 10)
big_timestamps <- seq(ymd_hms("2011-10-22 16:42:00"), by = 1000, length.out = 1e5)

# Report timing:
system.time(get_sm_data(1, 1, timestamps))
system.time(SMOStif(big_timestamps, 8, 23))
```
