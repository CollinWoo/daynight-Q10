...
SMOStif <- function(datevec, lon, lat){
  ...
  for(date in fileDates){
    ...
    fileRow <- data.frame(Surface = matchfiles[1], Subsurface = matchfiles[2])
    dateFiles[[toString(as.Date(date))]] <- fileRow
  }
  
  bind_rows(dateFiles, .id = "Date") %>% 
    distinct(Surface, .keep_all = TRUE) %>% 
    filter(!is.na(Surface)) ->  dateFiles
  ...
  if(nrow(dateFiles) > 0){
    for(value in 1:nrow(dateFiles)){
      dateFiles[value, 2] = extractSMOS(dateFiles[[value, 2]], lon, lat)
      dateFiles[value, 3] = extractSMOS(dateFiles[[value, 3]], lon, lat)
    }
  }
  print(dateFiles)
  return(dateFiles)
...
system.time(SMOStif(big_timestamps, 1, 1))
```
