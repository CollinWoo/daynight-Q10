...
yearR2 <- ggplot(year_Q10, aes(x = R2)) + geom_histogram(color= "black", fill = "white") + labs(title = "Year R2 Distribution") + geom_vline(aes(xintercept = median(R2)), linetype="dashed", color="red") + geom_text(aes(label = paste("Median =", round(median(R2), digits=3))), x = 0.15, y = 20, size = 3)
...
SMOSfiles <- function(date){
  ...
  if(!file.exists(matchfiles[1])){
    try(download.file(fileurls[1], destfile= paste0(matchfiles[1]), method = "curl"))
    try(download.file(fileurls[2], destfile= paste0(matchfiles[2]), method = "curl"))
  }
